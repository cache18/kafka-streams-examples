...
//@Configuration
public class FeedbackFixStream {
    ...
    public KStream<String, FeedbackMessage> kStreamFeedback(StreamsBuilder builder) {
        ...
        sourceStream.flatMap(splitWords())
                ...
                .branch(isGoodWord(), Branched.withConsumer(ks -> {
                    ...
                    ks.groupBy((k, v) -> v).count().toStream().to("t-commodity-feedback-six-good-count-word");
                }))
                .branch(isBadWord(), Branched.withConsumer(ks -> {
                    ...
                    ks.groupBy((k,v) -> v).count().toStream().to("t-commodity-feedback-six-bad-count-word");
                }));
