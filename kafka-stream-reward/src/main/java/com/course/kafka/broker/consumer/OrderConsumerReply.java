...
public class OrderConsumerReply {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(OrderConsumerReply.class);
    ...
    public OrderReplyMessage listen(ConsumerRecord<String, OrderMessage> consumerRecord) {
        ...
        var headerValue = ObjectUtils.isEmpty(headers.lastHeader("surpriseBonus"))
                ? "0"
        ...
        return new OrderReplyMessage("Order " + orderMessage.getOrderNumber() +
                ", item " + orderMessage.getItemName() +
                " processed");
    }
}
